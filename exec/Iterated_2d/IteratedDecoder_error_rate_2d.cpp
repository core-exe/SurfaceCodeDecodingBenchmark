...
vector<int> test_batch(int d, double p_eff, int mode = 0) {
    ...
    */
    ...
    auto decoder = Dc::Matching::IteratedDecoder(px, py, pz, pm, false, 4);
...
int main() {
    ...
    */
    ...
    auto path = std::filesystem::path(PROJECT_ROOT_PATH) / "exec/Iterated_2d/out/";
    string file_name;
    if(mode == 0) {
        file_name = "Iterated_2d_out_balance.txt";
    } else if(mode == 1) {
        file_name = "Iterated_2d_out_independent.txt";
    }
    ofstream file;
    ...
    for(auto d_it = d_list.begin(); d_it != d_list.end(); d_it++) {
        for(auto p_it = p_list.begin(); p_it != p_list.end(); p_it++) {
            ...
            printf("d = %3d, p = %6.4f, time spent: %12.5f\n", *d_it, *p_it, time_span.count());
            fflush(stdout);
        }
    }
    file.close();
